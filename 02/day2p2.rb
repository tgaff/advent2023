...
class Game
  ...
  def rgb_combo_possible?(r,g,b)
    return false unless color_quantity_possible?(:red, r)
    return false unless color_quantity_possible?(:green, g)
    return false unless color_quantity_possible?(:blue, b)

    true
  end

  def color_minimum(color)
    raise ArgumentError.new "invalid color #{color}" unless %i[red green blue].include?(color)

    @handfuls.map(&color).max
  end

  def power
    color_minimum(:red) * color_minimum(:green) * color_minimum(:blue)
  end

  private

  def color_quantity_possible?(color, value)
...
puts "Sum of powers = #{game_powers.sum}"